...
cleanup_kong() {
  ...
  # Remove uploaded plugin schemas (Phase 2)
  CP_ID="${KONNECT_CONTROL_PLANE_ID:-}"
  REGION="${KONNECT_REGION:-us}"
  if [ -n "$CP_ID" ]; then
    info "Removing custom plugin schemas from Konnect CP..."
    API_BASE="https://${REGION}.api.konghq.com/v2/control-planes/${CP_ID}/core-entities/plugin-schemas"
    ...
    for plugin in kong-did-interceptor kong-did-verifier kong-worm-logger; do
      ...
      if [ "$HTTP_CODE" = "204" ] || [ "$HTTP_CODE" = "200" ]; then
        success "Removed schema: $plugin"
      elif [ "$HTTP_CODE" = "404" ]; then
        info "Schema not found: $plugin (already removed)"
      else
        warn "Failed to remove schema $plugin (HTTP $HTTP_CODE)"
      fi
    done
  fi
}
