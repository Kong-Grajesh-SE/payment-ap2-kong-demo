#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
# AP2 Payment Demo — Automated Setup
# ═══════════════════════════════════════════════════════════════════
#
# Usage:
...
#   1. Checks prerequisites (Docker, decK, Node.js, Go)
...
#
# For Phase 2 (custom plugins), run with: ./setup.sh --phase2
# ═══════════════════════════════════════════════════════════════════
...
for arg in "$@"; do
  case "$arg" in
    ...
    --phase2)    PHASE2=true ;;
    --help|-h)
      echo "Usage: ./setup.sh [--check] [--skip-env] [--phase2]"
      echo "  --check     Check prerequisites only"
      echo "  --skip-env  Skip .env setup (already configured)"
      echo "  --phase2    Include custom plugin setup (build + upload schemas)"
      exit 0
      ;;
  esac
done
...
if $PHASE2; then
  check_cmd go || MISSING=$((MISSING + 1))
fi
...
if [ $MISSING -gt 0 ]; then
  ...
  if $PHASE2; then
    echo "  brew install go               # Go compiler (for custom plugins)"
  fi
  exit 1
fi
...
if $PHASE2; then
  prompt_if_empty "KONNECT_CONTROL_PLANE_ID" "Konnect Control Plane ID (for schema upload)"
fi
...
# ═══════════════════════════════════════════════════════════════════
# STEP 6: Phase 2 — Custom Plugins (optional)
# ═══════════════════════════════════════════════════════════════════
if $PHASE2; then
  header "Step 6: Custom Plugin Setup (Phase 2)"
  ...
  # Upload schemas to Konnect CP
  CP_ID="${KONNECT_CONTROL_PLANE_ID:-}"
  if [ -z "$CP_ID" ]; then
    warn "KONNECT_CONTROL_PLANE_ID not set — skipping schema upload"
    echo "  Set it in .env and run: ./scripts/upload-schemas.sh"
  else
    info "Uploading plugin schemas to Konnect CP..."
  ...
  echo ""
  warn "Custom plugins are built but NOT yet loaded on your DP."
  echo "  Add these to your existing Kong DP docker run/compose:"
  echo ""
  echo "  Env vars:"
  echo '    KONG_PLUGINS=bundled,kong-did-interceptor,kong-did-verifier,kong-worm-logger'
  echo '    KONG_PLUGINSERVER_NAMES=kong-did-interceptor,kong-did-verifier,kong-worm-logger'
  echo '    KONG_PLUGINSERVER_KONG_DID_INTERCEPTOR_START_CMD=/opt/kong/plugins/kong-did-interceptor'
  echo '    KONG_PLUGINSERVER_KONG_DID_INTERCEPTOR_QUERY_CMD=/opt/kong/plugins/kong-did-interceptor -dump'
  echo '    KONG_PLUGINSERVER_KONG_DID_VERIFIER_START_CMD=/opt/kong/plugins/kong-did-verifier'
  echo '    KONG_PLUGINSERVER_KONG_DID_VERIFIER_QUERY_CMD=/opt/kong/plugins/kong-did-verifier -dump'
  echo '    KONG_PLUGINSERVER_KONG_WORM_LOGGER_START_CMD=/opt/kong/plugins/kong-worm-logger'
  echo '    KONG_PLUGINSERVER_KONG_WORM_LOGGER_QUERY_CMD=/opt/kong/plugins/kong-worm-logger -dump'
  echo ""
  echo "  Volume mounts:"
  echo "    -v $(pwd)/plugins/kong-did-interceptor/kong-did-interceptor:/opt/kong/plugins/kong-did-interceptor"
  echo "    -v $(pwd)/plugins/kong-did-verifier/kong-did-verifier:/opt/kong/plugins/kong-did-verifier"
  echo "    -v $(pwd)/plugins/kong-worm-logger/kong-worm-logger:/opt/kong/plugins/kong-worm-logger"
  echo ""
  echo "  Then restart your DP and re-run:"
  echo "    deck gateway sync --konnect-token \$KONNECT_API_TOKEN --konnect-control-plane-name \$KONNECT_CONTROL_PLANE_NAME --select-tag ap2-agents config/kong.deck.yml"
  echo ""
fi

# ═══════════════════════════════════════════════════════════════════
# STEP 7: Verify Agent Routes Through Kong
# ═══════════════════════════════════════════════════════════════════
if $PHASE2; then
  header "Step 7: Verifying Agent Routes Through Kong"
else
...
# ═══════════════════════════════════════════════════════════════════
# STEP 8: Install Demo App Dependencies
# ═══════════════════════════════════════════════════════════════════
if $PHASE2; then
  header "Step 8: Installing Demo App Dependencies"
else
...
echo ""
if $PHASE2; then
  echo "  Phase 2: Custom plugins built. See above for DP configuration."
  echo ""
fi
